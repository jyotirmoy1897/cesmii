...
class DsEnv(gym.Env):
    ...
    def __init__(self, nb_jobs, nb_operations, nb_machines, nb_machines_types, instance_file=None):
        ...
        self.nb_type_max = np.max(self.nb_machines_types)
    ...
    def reset(self):
        ...
        self.total_perform_op_time_jobs = self.jobs_length
        # store the machine needed for all jobs
        self.needed_machine_jobs = np.zeros(self.nb_jobs, dtype=np.int)
        self.total_idle_time_jobs = np.zeros(self.nb_jobs, dtype=np.int)
        self.idle_time_jobs_last_op = np.zeros(self.nb_jobs, dtype=np.int)
    ...
    def _increase_time_step(self):
        ...
        #print("Current time step", self.current_time_step)
                ...
                if self.time_until_finish_current_op_jobs[job] == 0:
                    ...
                    if self.todo_time_step_job[job] < self.nb_operations:
                        ...
                        self.state[job][4] = max(
                            0, np.min(self.time_until_available_machine[self.needed_machine_jobs[job]])-difference) / self.max_time_op
                    else:
        ...
        for job in range(self.nb_jobs):
            ...
            while i < self.nb_operations and self.solution[job][i] != -1:
                ...
                start_sec = self.start_timestamp + \
                    self.solution[job][i]*3600
                # print(start_sec, self.start_timestamp)
                finish_sec = start_sec + \
                    self.instance_matrix[job][i][1]*3600
                dict_op["Start"] = datetime.datetime.fromtimestamp(
                    start_sec)
                dict_op["Finish"] = datetime.datetime.fromtimestamp(
                    finish_sec)
    ...
    def makespan(self):
        ...
        for job in range(self.nb_jobs):
            i = 0
            while i < self.nb_operations and self.solution[job][i] != -1:
                start_sec = self.start_timestamp + self.solution[job][i]*3600
                finish_sec = datetime.datetime.fromtimestamp(
                    start_sec + self.instance_matrix[job][i][1]*3600)
                i += 1
            if job == 0:
